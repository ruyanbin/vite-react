...
const SearchForm = ({ columns }: { columns: Array<FormItemProps> }) => {
        ...
        <Row gutter={24}>
            {columnsToShow.map((item) => {
                return (
                    <Col key={item.dataIndex} span={8}>
                        <Form.Item name={item.dataIndex} label={item.title} rules={item.rules || [{ required: false, message: `请输入${item.title}` }]}>
                          <FormItem {...item} />
                        </Form.Item>
                    </Col>
                )
            ...
            <Col span={8} style={{ textAlign: 'right' }}>
                <Form.Item>
                    ...
                    <Button style={{ marginLeft: 8 }} onClick={() => { setExpend(!expend)}}>展开/收起</Button>
                </Form.Item>
            </Col>
        </Row>
       </Form>
